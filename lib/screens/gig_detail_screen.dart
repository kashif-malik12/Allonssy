...
class _GigDetailScreenState extends State<GigDetailScreen> {
  ...
  double? _priceFromContent(String raw) {
    final patterns = [
      RegExp(r'(?:price|budget|rate)\s*:\s*(\d+(?:[.,]\d{1,2})?)', caseSensitive: false),
      RegExp(
        r'(?:price|budget|rate)\s*:\s*(?:eur|euro|â‚¬|\$)\s*(\d+(?:[.,]\d{1,2})?)',
        caseSensitive: false,
      ),
      RegExp(r'(?:eur|euro|â‚¬)\s*(\d+(?:[.,]\d{1,2})?)', caseSensitive: false),
  ...
  List<Map<String, dynamic>> _questionRoots() {
    return _qaComments.where((row) => row['parent_comment_id'] == null).toList();
  }
        ...
        if (canSendOffer) ...[
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              onPressed: () => context.push('/offer-chat/post/${p.id}/user/${p.userId}'),
