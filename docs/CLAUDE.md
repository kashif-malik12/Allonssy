...
| 2026-03-18 | Feed UX: collapsible filters (marketplace + gigs), single feed-level mute for video feed, gradient tap-through fix (`IgnorePointer`), pull-to-refresh via `PageController` listener, deferred media loading with `VisibilityDetector` (50% threshold), session-only quick filter chips (mobile overlay + web top bar), FABs hidden on video feed page. |
| 2026-03-18 | Sharing module: `share_plus`, `ShareButton` widget in app bar of 3 detail screens (marketplace product, gig, food). Android App Links + iOS Universal Links. `.well-known` files in `web/`. Caddy updated to serve them with correct Content-Type. |
| 2026-03-18 | Legal pages: About Us, Terms & Conditions, Privacy Policy. Routes `/about` `/terms` `/privacy` public (no auth). Entry points: mobile account sheet, web app bar menu, login card footer, register card footer. Company: Tradister SAS, SIREN 988 318 945, Ris-Orangis, France. |
| 2026-03-18 | Price range feature: `market_price_max` column added to DB + migrated to VPS. `Post` model + `PostService` updated. Create post screen shows Single/Range toggle for marketplace and gigs. All 5 display locations (post_card, marketplace, gigs, detail screens) show `EUR X – EUR Y` format for ranges. |
| 2026-03-19 | Banned emails: `banned_emails` table + BEFORE INSERT trigger on `auth.users`. Admin UI with paginated list, unban, manual add, and post-delete ban prompt. `TabController` extended to 8 tabs. |
| 2026-03-19 | White hover fix: chips use `color: WidgetStateProperty` (no `overlayColor` on ChipThemeData). FilledButton uses dark tint overlay instead of white flash. OutlinedButton gets teal overlay. |
| 2026-03-19 | French category translations: all 5 category label functions (`market`, `service`, `food`, `restaurant`, `business`) accept `isFrench` param. All list/detail/create screens pass locale. Post types localized via `PostTypeX.localizedLabel()`. `create_post_launcher.dart` fully translated. Feed badges + filters updated. |
| 2026-03-19 | Chat read receipt ticks: `Icons.done` (grey=sent) / `Icons.done_all` (teal=seen) + timestamp on every message bubble. `chat_screen.dart` and `offer_chat_screen.dart` updated. Uses existing `read_at` column from DB. |
| 2026-03-19 | Google Sign-In Play Store fix: Play Store delivery SHA-1 added to Google Cloud Console Android OAuth. Play Store SHA-256 added to Firebase. Second Android OAuth client created for debug SHA-1. Version 1.0.1+2. |
| 2026-03-19 | App name fix: restored `android:label="Allonssy!"` in AndroidManifest (was stripped to "Allonssy" in pre-release cleanup). Version 1.0.3+4 built and uploaded to Play Store internal testing. |
| 2026-09-12 | Web OAuth redirect & root route fix: handled `/?code=...` with holding state for PKCE token exchange and resolved unhandled root route GoException. |
| 2026-09-12 | 2-Tier Business & Professional Categorization: Replaced flat business categories with 7 main categories and 26 subcategories across profiles, directory screen ("Professionals and Service Providers"), profile detail, and DB. Added `business_subtype` column and index on VPS Supabase. |
| 2026-09-12 | Feed share badge & visibility refinement: Replaced awkward sentence pills with clean, localized `_buildShareBadge` (`Shareable`, `Share: Followers`, `Share: Connections`) and localized `[LOCAL]`/`[PUBLIC]`. |
| 2026-09-12 | Release 1.2.0+7: Bumped version to `1.2.0+7`, passed comprehensive pre-live QA, deployed live to web (`https://app.allonssy.com`), and built Android release App Bundle. |
| 2026-09-20 | Release 1.3.0+8: Saved listings & bookmarks, item condition & original price discounts, item status (available/reserved/sold), favorite businesses, profile listings tabs, categorized search, in-app feedback & bug reporting, automated push notifications pipeline, ProGuard/R8 audit for MainActivity & MethodChannel, and pre-launch stress testing. |
